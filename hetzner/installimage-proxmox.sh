...
installimage_file="/root/images/Debian-1207-bookworm-amd64-base.tar.gz"
...
if [ "$OS" == "PBS" ] ; then
  if [ ! -f postinstall_file="/root/pbs" ] ; then
    wget "https://raw.githubusercontent.com/lupetalo/xshok-proxmox-8/master/hetzner/pbs" -c -O /root/pbs
  fi
  postinstall_file="/root/pbs"
else
  if [ ! -f postinstall_file="/root/pve" ] ; then
    wget "https://raw.githubusercontent.com/lupetalo/xshok-proxmox-8/master/hetzner/pve" -c -O /root/pve
  fi
